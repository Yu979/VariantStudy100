...
def make_model_and_train(heads, input_sequences, all_outputs, fit_epochs, hic_num, mp_q):
    ...
    try:
        ...
        safe_save(optimizers["our_expression"].get_weights(), p.model_path + "_opt_expression")
        if hic_num > 0:
...
def check_perf(mp_q):
    ...
    try:
        ...
        training_spearman = evaluation.eval_perf(p, our_model, heads, train_eval_chr_info,
                                                 False, current_epoch, "train", one_hot)
        # training_spearman = 0
        print(f"Valid set {len(valid_info)}")
        test_spearman = evaluation.eval_perf(p, our_model, heads, valid_info,
